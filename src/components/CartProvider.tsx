...
type CouponValidation={valid?:boolean;code?:string;discount_amount?:number;message?:string};
...
export function CartProvider({children}:{children:React.ReactNode}){const [items,setItems]=useState<CartItem[]>([]);const [ready,setReady]=useState(false);const [coupon,setCoupon]=useState<CouponState>(null);const [couponBusy,setCouponBusy]=useState(false);
 ...
 const applyCoupon=useCallback(async(code:string)=>{const clean=code.trim().toUpperCase();if(!clean){setCoupon(null);localStorage.removeItem(COUPON_KEY);return false;}setCouponBusy(true);const {data,error}=await supabase.rpc('validate_coupon',{p_code:clean,p_subtotal:subtotal});setCouponBusy(false);const r=data as CouponValidation|null;if(error||!r?.valid){setCoupon({code:clean,discount:0,message:r?.message||error?.message||'Kupon geçersiz'});return false;}setCoupon({code:r.code||clean,discount:Number(r.discount_amount)||0});localStorage.setItem(COUPON_KEY,r.code||clean);return true;},[subtotal]);
 useEffect(()=>{if(!ready||!coupon?.code)return;applyCoupon(coupon.code);},[ready,coupon?.code,applyCoupon]);
 const clearCoupon=useCallback(()=>{setCoupon(null);localStorage.removeItem(COUPON_KEY)},[]);
 const value=useMemo<CartContextValue>(()=>({items,count:cartCount(items),subtotal,coupon,discount:coupon?.discount||0,total:Math.max(0,subtotal-(coupon?.discount||0)),couponBusy,addItem:(item,quantity=1)=>setItems(current=>{const existing=current.find(x=>x.id===item.id);if(existing)return current.map(x=>x.id===item.id?{...x,quantity:clampQuantity(x.quantity+quantity,item.stock_quantity),stock_quantity:item.stock_quantity,price:item.price}:x);const q=clampQuantity(quantity,item.stock_quantity);return q?[...current,{...item,quantity:q}]:current;}),setQuantity:(id,quantity)=>setItems(current=>current.map(x=>x.id===id?{...x,quantity:clampQuantity(quantity,x.stock_quantity)}:x).filter(x=>x.quantity>0)),removeItem:(id)=>setItems(current=>current.filter(x=>x.id!==id)),clearCart:()=>{setItems([]);clearCoupon()},applyCoupon,clearCoupon}),[items,subtotal,coupon,couponBusy,applyCoupon,clearCoupon]);return <CartContext.Provider value={value}>{children}</CartContext.Provider>}
