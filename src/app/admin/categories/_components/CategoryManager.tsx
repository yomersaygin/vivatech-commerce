...
export default function CategoryManager(){
  ...
  async function load(){const [{data,error},{data:products}]=await Promise.all([supabase.from('categories').select('id,name,slug,parent_id,image_url,seo_title,seo_description,is_active').order('name'),supabase.from('products').select('category_id')]); if(error)setMessage(error.message); else setItems((data??[]) as Category[]);const counts:Record<string,number>={};for(const row of products??[]){if(row.category_id)counts[row.category_id]=(counts[row.category_id]??0)+1}setUsage(counts)}
  useEffect(()=>{load()},[]);
  ...
  async function submit(e:FormEvent){e.preventDefault();setBusy(true);setMessage('');try{const wasEditing=Boolean(editing);const payload={name:form.name.trim(),slug:slugify(form.slug||form.name),parent_id:form.parent_id||null,image_url:form.image_url.trim()||null,seo_title:form.seo_title.trim()||null,seo_description:form.seo_description.trim()||null,is_active:form.is_active};const q=editing?supabase.from('categories').update(payload).eq('id',editing):supabase.from('categories').insert(payload);const {error}=await q;if(error)throw error;reset();await load();setMessage(wasEditing?'Kategori güncellendi.':'Kategori eklendi.')}catch(err){setMessage('Hata: '+(err instanceof Error?err.message:String(err)))}finally{setBusy(false)}}
